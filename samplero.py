...
class CustomEmbeddingModel(PreTrainedModel):
    def __init__(self, model_name, embedding_model, device="cuda"):
        super().__init__(config=AutoConfig.from_pretrained(model_name))
        self.transformer = AutoModelForCausalLM.from_pretrained(model_name)
        self.transformer.set_input_embeddings(embedding_model)
    ...
    def generate(self, tokenizer, s: str, **kwargs):
        input_bytes = torch.tensor(list(s.encode('utf-8')), dtype=torch.long).to("cuda").unsqueeze(0)
        import pdb; pdb.set_trace()
        return self.transformer.generate(input_ids=input_bytes, **kwargs)
